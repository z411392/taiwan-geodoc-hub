...
import { Regions } from "@/taiwan-geodoc-hub/infrastructure/regions"
...
const useQueryParams = () => {
    ...
    const initStatus = (): ValueStatuses | "all" => {
        const status = searchParams.get("status")
        return Object.values(ValueStatuses).includes(status as ValueStatuses)
            ? (status as ValueStatuses)
            : "all"
    }
    ...
    const [region, setRegion] = useState<Regions | "all">(initRegion())
    const [status, setStatus] = useState<ValueStatuses | "all">(initStatus())
...
export const ValuesTable = () => {
    ...
    const t = useTranslations(Pages.Values)
    const {
        region,
        setRegion,
        status,
                    ...
                    <div className="flex flex-col gap-4">
                        <form className="flex flex-wrap gap-4">
                            ...
                            <Select
                                ...
                                value={status}
                                onValueChange={(value) =>
                                    setStatus(value as ValueStatuses)
                                }
                            >
                                <SelectTrigger className="w-[180px]">
                                    <SelectValue
                                        ...
                                        <TableRow key={value.id}>
                                            ...
                                            <TableCell>
                                                <Badge
                                                    variant={
                                                        value.status ===
                                                        ValueStatuses.Success
                                                            ? "success"
                                                            : value.status ===
                                                                ValueStatuses.Failed
                                                              ? "error"
                                                              : "info"
                                                    }
                                                >
