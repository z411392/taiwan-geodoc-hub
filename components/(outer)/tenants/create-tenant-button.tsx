...
export default function CreateTenantButton() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    alert(t("messages.tenant-created"))
  }
    ...
    <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("forms.create-tenant.title")}</DialogTitle>
            <DialogDescription>
              {t("forms.create-tenant.description")}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="name" className="text-left">
                  {t("forms.create-tenant.inputs.name.label")}
                </Label>
                <Input
                  id="name"
                  placeholder={t("forms.create-tenant.inputs.name.placeholder")}
                  value={tenantName}
