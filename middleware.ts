...
import { APIRoutes, Pages } from "@/taiwan-geodoc-hub/infrastructure/routes"
import { Locale } from "@/taiwan-geodoc-hub/infrastructure/cookies"
import { getLocale } from "next-intl/server"
import { PublicPages } from "@/taiwan-geodoc-hub/infrastructure/routes"
...
const reporters = [
    {
        log: ({ args, tag, date }) => console.log(tag, date, ...args),
    } as ConsolaReporter,
]

const invalidTokenLogger = createConsola({ reporters }).withTag(
    `onTokenInvalid`,
)
...
const onTokenInvalid = async (
    ...
    invalidTokenLogger.info({ reason, next })
    redirectURI.searchParams.set("next", base62.encode(next))
    return NextResponse.redirect(redirectURI)
}

const validTokenLogger = createConsola({ reporters }).withTag(`onTokenValid`)
...
const onTokenValid = async (
    ...
    validTokenLogger.info({ redirectedTo })
    return redirectToPath(request, redirectedTo, {
        shouldClearSearchParams: true,
    })
}

const errorLogger = createConsola({ reporters }).withTag(`onError`)

const onError = async (request: NextRequest, thrown: unknown) => {
    const error =
