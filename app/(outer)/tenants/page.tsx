...
import { onListingTenants } from "@/lib/modules/access-managing/presentation/ui/on-listing-tenants"
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className="bg-amber-50 text-amber-600">
                {t(`statuses.tenant.${tenant.status}`)}
              </Badge>
            </div>
            <p className="text-sm text-gray-500">{t(`roles.${tenant.role}`)}</p>
          </div>
        </div>
      </CardContent>
    </Card>
